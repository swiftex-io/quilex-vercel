...
const SpotTrading: React.FC = () => {
  ...
  const { balances, placeOrder, tradeHistory, openOrders, cancelOrder, activePair, setActivePair, favorites, toggleFavorite } = useExchangeStore();
  ...
  const StarIcon = ({ symbol, filled, className }: { symbol: string, filled: boolean, className?: string }) => (
    <button 
      ...
      className={`transition-all duration-300 transform active:scale-125 ${className || ''} ${filled ? 'text-amber-400' : 'text-zinc-700 hover:text-zinc-400'}`}
    >
    ...
    <div className="flex h-[calc(100vh-64px-32px)] bg-black overflow-hidden border-t border-zinc-900">
        ...
        <div className="flex-1 overflow-y-auto custom-scrollbar bg-black/10">
          ...
          {availablePairs.length === 0 && (
            <div className="py-20 text-center px-6">
              <p className="text-[10px] font-bold text-zinc-700 uppercase tracking-widest leading-loose">No pairs found</p>
            </div>
          )}
        </div>
      </div>
      ...
      <div className="flex-1 flex flex-col min-0 h-full overflow-hidden border-r border-zinc-900">
        ...
        <div className="h-[60px] border-b border-zinc-900 flex items-center px-4 gap-8 bg-black shrink-0 relative z-50">
          <div className="flex items-center gap-4 relative" ref={pairSelectorRef}>
            <div 
            ...
            <button 
              onClick={(e) => { e.stopPropagation(); toggleFavorite(activeBase); }}
              className={`w-8 h-8 rounded-full flex items-center justify-center transition-all active:scale-90 ${
                favorites.includes(activeBase) 
                  ? 'bg-amber-400/20 text-amber-400' 
                  : 'bg-zinc-800 text-zinc-500 hover:bg-zinc-700 hover:text-zinc-300'
              }`}
              title={favorites.includes(activeBase) ? "Remove from favorites" : "Add to favorites"}
            >
              <svg width="14" height="14" viewBox="0 0 24 24" fill={favorites.includes(activeBase) ? "currentColor" : "none"} stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round">
                <polygon points="12 2 15.09 8.26 22 9.27 17 14.14 18.18 21.02 12 17.77 5.82 21.02 7 14.14 2 9.27 8.91 8.26 12 2"/>
              </svg>
            </button>
      ...
      {/* Right Column Group (Orderbook & Trade) - Responsive 2-column on 1500px+ */}
          ...
          <div className="h-auto min-[1500px]:flex-1 min-[1500px]:overflow-y-auto custom-scrollbar px-4 space-y-4 pt-4 pb-4">
              ...
              {orderType === 'tpsl' ? (
                <div className="relative shrink-0 flex items-center" ref={tpslTypeRef}>
                  <button 
                    ...
                    className="text-[10px] font-black text-zinc-500 uppercase tracking-widest min-[1500px]:min-w-[60px] min-w-[50px] flex items-center gap-1.5 hover:text-zinc-300 transition-colors"
                  >
                    {tpslExecutionType} 
                  ...
                  {isTpslTypeDropdownOpen && (
                    <div className="absolute top-full left-0 mt-2 w-32 bg-[#1a1a1a] border border-zinc-800 rounded-lg shadow-2xl z-[110] py-1">
                      {(['market', 'limit'] as const).map(type => (
                        <button 
                          key={type}
                          onClick={() => { setTpslExecutionType(type); setIsTpslTypeDropdownOpen(false); }}
                          className={`w-full text-left px-3 py-2.5 text-[11px] font-black uppercase tracking-widest hover:bg-zinc-800 transition-colors ${tpslExecutionType === type ? 'text-white bg-zinc-800' : 'text-zinc-500'}`}
                        >
                          {type}
                        </button>
                      ))}
            ...
            >
              <div className="relative h-8 flex items-center">
                ...
                <div className="absolute top-1/2 left-0 right-0 flex justify-between -translate-y-1/2 pointer-events-none px-[1px]">
                  {[0, 25, 50, 75, 100].map(p => (
                    <div key={p} className={`w-2 h-2 rounded-full border-2 border-zinc-900 transition-colors ${percent >= p ? (side === 'buy' ? 'bg-[#00d18e]' : 'bg-[#ff4d4f]') : 'bg-zinc-700'}`} />
                  ))}
                </div>
                <input 
                  type="range" 
                ...
                />
                <div 
                  className={`absolute -top-7 -translate-x-1/2 pointer-events-none transition-all duration-150 flex flex-col items-center ${isSliderHovered ? 'opacity-100 scale-100' : 'opacity-0 scale-95'}`}
                  style={{ left: `${percent}%` }}
                >
                  <div className="bg-white text-black text-[12px] font-black px-2 py-0.5 rounded shadow-xl leading-none">
                    {percent}%
                  </div>
              ...
              <div className="flex justify-between text-[10px] font-bold text-zinc-500 px-0.5 uppercase tracking-tight -mt-1 select-none">
                ...
                <span className="w-6 text-center text-zinc-600">25%</span>
                <span className="w-6 text-center text-zinc-600">50%</span>
                <span className="w-6 text-center text-zinc-600">75%</span>
            ...
            <div className="grid grid-cols-4 gap-2">
               {[25, 50, 75, 100].map(pct => (
                 <button 
                  ...
                  className={`py-1.5 bg-zinc-900/80 border border-zinc-800 rounded-lg text-[12px] font-bold transition-all ${percent === pct ? 'bg-zinc-700 text-white border-zinc-500' : 'text-zinc-500 hover:text-white hover:bg-zinc-800'}`}
                 >
                  {pct}%
                 </button>
               ))}
            ...
            {orderType !== 'tpsl' && (
              <div className="pt-2">
                <label className="flex items-center gap-3 cursor-pointer group w-fit select-none">
                  <div className={`w-5 h-5 rounded border transition-all flex items-center justify-center ${showTPSL ? 'bg-white border-white' : 'border-zinc-700 group-hover:border-zinc-500'}`}>
                    {showTPSL && <svg className="w-3.5 h-3.5 text-black" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="4"><path d="m5 13 4 4L19 7"/></svg>}
                    <input type="checkbox" className="hidden" checked={showTPSL} onChange={() => setShowTPSL(!showTPSL)} />
                  </div>
                  <span className="text-[13px] font-bold text-zinc-400 group-hover:text-zinc-200 transition-colors uppercase tracking-tight">TP/SL</span>
                </label>
                ...
                <div className={`overflow-hidden transition-all duration-300 ease-in-out ${showTPSL ? 'max-h-40 opacity-100 mt-4 space-y-3' : 'max-h-0 opacity-0'}`}>
                  ...
                  <div className="flex items-center border border-zinc-800 bg-[#111] rounded-xl px-3 h-11 focus-within:border-zinc-500 transition-all min-w-0">
                    <span className="text-[10px] font-black text-zinc-500 uppercase tracking-widest min-[1500px]:min-w-[60px] min-w-[50px] shrink-0">TP Price</span>
                    <input 
                      type="number" 
                      value={tpInput}
                  ...
                  {/* Stop Loss Input */}
                  <div className="flex items-center border border-zinc-800 bg-[#111] rounded-xl px-3 h-11 focus-within:border-zinc-500 transition-all min-w-0">
                    <span className="text-[10px] font-black text-zinc-500 uppercase tracking-widest min-[1500px]:min-w-[60px] min-w-[50px] shrink-0">SL Price</span>
                    <input 
                      type="number" 
                      value={slInput}
