      ...
      <div className="max-w-7xl mx-auto px-8 py-16">
        ...
        <div className="mb-32 relative text-left">
          <div className="flex justify-between items-center mb-12">
            <div>
               <h2 className="text-3xl font-black tracking-tighter mb-2">{isAffiliate ? 'Partner Progression' : 'Milestone Roadmap'}</h2>
              ...
              <div className="relative cursor-help ml-2">
                ...
                <div className="absolute bottom-full right-0 mb-3 w-64 p-4 bg-zinc-900 border border-white/10 rounded-xl shadow-2xl opacity-0 invisible group-hover/multiplier:opacity-100 group-hover/multiplier:visible transition-all z-50 pointer-events-none">
                  ...
                  <p className="text-[11px] text-zinc-400 font-medium leading-relaxed text-left">
                    Your direct share of trading fees. Higher ranks amplify your passive revenue significantly.
                  </p>
            ...
            {tiers.map((t, index) => {
              ...
              return (
                <div 
                  ...
                  style={{
                    background: `linear-gradient(90deg, ${data.color}25 0%, ${data.color}05 50%, transparent 100%)`
                  }}
                >
                  {/* Smooth Radial Side Glow - More prominent */}
                  <div 
                    className="absolute left-0 top-0 bottom-0 w-[120px] pointer-events-none transition-opacity duration-1000 opacity-40 group-hover:opacity-70"
                    style={{
                      background: `radial-gradient(circle at -20% 50%, ${data.color}50 0%, transparent 80%)`
                    }}
                  ></div>

                  {/* Decorative High-Contrast Side Flash - Thicker & Sharper */}
                  <div 
                    className="absolute left-0 top-0 bottom-0 w-[4px] transition-all duration-700 opacity-60 group-hover:opacity-100"
                    style={{ 
                      backgroundColor: data.color, 
                      boxShadow: `0 0 25px ${data.color}A0, 0 0 10px ${data.color}` 
                    }}
                  ></div>

                  <div className="flex justify-between items-start mb-8 relative z-10">
                    <div 
                      className={`text-6xl font-black tracking-tighter transition-all duration-700 ${isCurrent ? 'opacity-50 scale-110' : 'opacity-10 group-hover:opacity-25'}`}
                      style={{ color: data.color }}
                    >
                      {(index + 1).toString().padStart(2, '0')}
                    </div>
                    {isCurrent && (
                      <span className="bg-white/10 backdrop-blur-md border border-white/20 text-white px-4 py-1.5 rounded-full text-[9px] font-black uppercase tracking-widest shadow-2xl animate-pulse">
                        Current Rank
                      </span>
                    )}
                  </div>
                  ...
                  <div className="flex-1 relative z-10">
                    ...
                    <div className="mb-8">
                      ...
                      <div className="text-4xl font-black tracking-tighter text-white group-hover:text-white/100 transition-colors">
                        {data.avgEarn}
                      </div>
                    </div>
                  </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-5 gap-8 mb-8">
          <div className="lg:col-span-3 bg-[#0d0d0d] border border-white/5 rounded-2xl p-10 shadow-2xl relative overflow-hidden text-left">
            <h3 className="text-2xl font-black mb-10 tracking-tight">{isAffiliate ? 'Partnership' : 'Status'} Perks Matrix</h3>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-5 gap-8 items-start">
          <div className="lg:col-span-3 bg-[#0d0d0d] border border-white/5 p-8 rounded-2xl shadow-2xl relative overflow-hidden text-left">
             <div className="relative z-10">
                ...
                <p className="text-zinc-500 font-medium mb-8 max-w-sm text-[13px]">Your elite {isAffiliate ? 'partner' : 'invitation'} link tracks everything.</p>
          ...
          <div className="lg:col-span-2 bg-[#0d0d0d] border border-white/5 rounded-2xl p-8 text-left">
             ...
             <div className="space-y-2">
                {recentEvents.length > 0 ? recentEvents.map((ev, i) => (
                  <div key={i} className="flex items-center justify-between p-4 bg-black/40 rounded-xl border border-white/5 animate-in slide-in-from-bottom-4 fade-in duration-500">
                    <div className="flex items-center gap-3">
                      <div className="w-9 h-9 bg-zinc-900 rounded-full flex items-center justify-center overflow-hidden border border-white/10">
                        <img src={`https://api.dicebear.com/7.x/avataaars/svg?seed=${ev.user}`} alt="avatar" className="w-full h-full object-cover" />
                      </div>
                      <div>
                  ...
                  <div className="py-12 text-center text-zinc-700 text-[10px] font-bold uppercase tracking-widest">Scanning network...</div>
                )}
             </div>
          </div>
        </div>
